import maskFunction, { defaultMaskOptions, maskString, maskTextOptions} from '.'
...
describe('Mask sensitive data', () => {
  describe('Object', () => {
    ...
    it('should mask object contains build-in objects like Date, RegExp, Function', () => {
      const dataToBeMasked = {
        date: new Date('2023-05-01T00:00:00.000Z'),
        regexp: new RegExp('1b20f409-0f80-415b-a547-8557b1e7d028', 'i'),
        [Symbol('token').toString()]: 'dc4abd72-7d64-404d-a9b5-ab3ce90467fb',
      }

      expect(maskFunction(dataToBeMasked)).toEqual({
        'Symbol(token)': 'dc4abd##########################67fb',
        date: new Date('2023-05-01T00:00:00.000Z'),
        regexp: /1b20f4##########################d028/i,
      })
    })
  })
...
like foo@bar since this is not an email-address. `)
    ...
    it('should mask string based on default config', () => {
      expect(maskString('john.doe@example.com')).toEqual('john.d##########.com')
    })

    it('should mask string with custom config', () => {
      const textToBeMasked = 'john.doe@example.com'
      expect(
        maskString('john.doe@example.com', {
          maskSymbol: '#',
          maxCharsToMask: textToBeMasked.length,
          visibleCharsFromStart: 0,
          visibleCharsFromEnd: 0,
        } as maskTextOptions)
      ).toEqual('####################')
